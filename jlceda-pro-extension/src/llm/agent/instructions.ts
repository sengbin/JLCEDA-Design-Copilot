...
const BUILT_IN_AGENT_SYSTEM_INSTRUCTIONS: string = [
	...
	'',
	'调用方法：',
	'1. 传入 keyword（必填），必须使用空格分隔，优先使用常用简称 + 封装/大小 + 关键参数，尽量短但要足够区分器件；不必扩写为正式品类全称，例如 LED 无需扩写为发光二极管。只有电阻、电容、电感这类带阻值/容值/感值的器件，在关键词包含数值参数时才必须带单位符号，例如电阻写 1kΩ，电容写 100nF，电感写 10uH；禁止写成 1k、100n、10u。',
	'2. 工具搜索并展示面板，申请用户确认选择。',
	'3. 用户单击确定后，工具返回包含 uuid 和 libraryUuid 的选中器件信息。',
	'4. 若需要把已选器件真正放到原理图中，必须调用 component_place 完成放置。',
	'',
	'参数规则：',
	'- `keyword`：器件搜索关键词，必填；仅对电阻、电容、电感这类带阻值/容值/感值的器件，若包含数值参数则必须带单位符号，禁止使用 1k、100n、10u 这类缺少单位的写法。',
	'- `limit`：返回候选数量上限，可选，默认 20。',
	'',
	'结果处理：',
	'- ok=true 时：selectedCandidate 包含 uuid、libraryUuid、name、footprintName 等字段，且该结果表示用户已最终确认器件型号。',
	'- 一旦用户确认 selectedCandidate，后续必须以该器件为准；即使它与你的预期、经验或先前计划不一致，也不得再次调用 component_select 要求用户重选，也不得自行改选其他候选器件。只有用户明确要求更换器件时，才允许重新选型。',
