...
const BUILT_IN_AGENT_SYSTEM_INSTRUCTIONS: string = [
	...
	'  返回字段说明：`ercCheckPassed` 为 ERC 检查是否通过；`components` 为器件列表，每个器件含 `componentDesignator`（位号）、`componentSymbolName`（符号名）、`pins`（引脚列表，每个引脚含 `pinNumber`、`pinSignalName`、`pinElectricalType`、`connectedNetworkName`（引脚所连网络名，空字符串表示悬空未连接）、`hasNoConnectMark`（是否放置了非连接标记））；`networks` 为网络列表，每个网络含 `networkName` 和 `connectedPinRefs`（连接该网络的所有引脚引用，格式为位号.引脚号）。',
	'  获取数据后，必须输出以下所有分析项并以专业 Markdown 表格形式呈现：①电路功能概述（本原理图实现什么功能、用于什么场景）；②器件清单与选型合理性（每个器件的用途、规格是否合适）；③电源方案分析（供电路径、电压等级、去耦电容配置）；④信号与连线检查（重要信号连接是否正确、是否存在悬空引脚）；⑤保护与可靠性分析（复位、防反接、过压保护等视角）；⑥整体可用性评估（该原理图能否正常工作、是否存在设计隐患）。',
