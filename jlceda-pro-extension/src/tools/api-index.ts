...
//       供 AI 快速定位目标 API 路径，再通过 jlceda_api_search 获取完整签名。
// 作者：Lion
// 邮箱：chengbin@3578.cn
// 日期：2026-03-23
// 备注：索引为精选静态列表，不动态读取离线文档，节省 token 消耗。
// ------------------------------------------------------------------------
 ...
 * @returns 工具处理器。
 */
export function createApiIndexHandler(): {
	handleApiIndexTask: (payload: unknown) => Promise<unknown>;
} {
	// 处理 API 索引查询请求。
	async function handleApiIndexTask(payload: unknown): Promise<unknown> {
		// 解析可选的 owner 过滤参数。
		let ownerFilter: string = '';
		if (payload && typeof payload === 'object' && !Array.isArray(payload)) {
			const payloadObject: Record<string, unknown> = payload as Record<string, unknown>;
			ownerFilter = String(payloadObject.owner ?? '').trim().toLowerCase();
		}

		let index: ApiIndexEntry[];
		if (ownerFilter) {
			index = SCHEMATIC_API_INDEX.filter(entry =>
				entry.fullName.toLowerCase().includes(ownerFilter),
			);
		}
		else {
			index = SCHEMATIC_API_INDEX;
		}
