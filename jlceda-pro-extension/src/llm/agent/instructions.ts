...
function buildAgentSystemInstructions(): string {
	return [
		...
		'',
		'### 调用规范',
		'1. 使用 `argsJson` 字段传入 API 参数：将全部参数按 `signatureText` 顺序放入数组并序列化为 JSON 字符串。例如 `argsJson: "[false,false,true]"`，或包含对象: `argsJson: "[{\\"uuid\\":\\"abc\\"},100,100]"`。无参数时传 `"[]"`。',
		'2. **signatureText 到 argsJson 的映射规则**：',
		'   - `signatureText` 中每个参数名对应 argsJson 数组里的一个元素，顺序严格一致。',
		...
		'     `argsJson: "[{\\"libraryUuid\\":\\"xxx\\",\\"uuid\\":\\"yyy\\"},100,100,null,null,null,true,true]"`',
