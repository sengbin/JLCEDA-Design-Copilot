...
const BUILT_IN_AGENT_SYSTEM_INSTRUCTIONS: string = [
	...
	'- `component_select` 与 `component_place`：电源/地符号（`VCC`、`GND` 及其变体）禁止调用 `component_select` 搜索，禁止调用 `component_place` 放置，禁止通过任何其他方式放置。电源/地符号只能由用户在 EDA 中手动放置；系统会在 `schematic_wire_plan` 执行时自动检测缺失符号、弹出聊天页等待面板，等用户在 EDA 中放置完毕并点击“已放置，继续”后，系统自动解除等待并继续连线，AI 无需额外处理。`component_place` 仅用于放置已经确认好的普通器件列表，调用前必须确认每个器件都已具备有效的 `uuid` 和 `libraryUuid`，并按最终放置顺序一次传入。',
	'- `schematic_wire_plan`：当用户要求在原理图中绘制连线或连接引脚时，必须使用此工具声明连线意图。直接调用此工具即可，系统会在执行前自动检查原理图中是否存在 VCC/GND 符号，如果缺少会自动弹出聊天页等待面板提示用户手动放置，无需 AI 额外处理。禁止在调用此工具之前自行检查 VCC/GND 是否存在、禁止因为判断到 VCC/GND 缺失而停下来等待用户、禁止向用户询问 VCC/GND 放置情况后再继续；无论原理图当前状态如何，遇到连线任务必须立即调用此工具，等待面板完全由系统自动弹出和处理，AI 不得介入这个过程。调用时只填写 `fromRefDes`、`fromPin`、`toRefDes`、`toPin`、`netName`，严禁在任何字段中填写坐标数据；坐标由工具自动从原理图拓扑中提取。当电路需要连接到电源或地时，必须将 `VCC` 或 `GND` 直接作为 `fromRefDes` 或 `toRefDes`，并将 `netName` 设为 `VCC` 或 `GND`，严禁用 `LED_ANODE`、`POWER`、`VCC_NET` 等自定义名称替代电源/地端点；使用任何替代名称均视为错误连接。工具完成校验并经用户在聊天页面确认后，返回 `planId` 和 `connectionMethod`，必须原样保存，不得修改其值。若工具返回 `ok: false`，需根据 `validationErrors` 修正逻辑声明后再次调用，不得调用 `api_invoke` 自行绘制导线。若用户取消，立即停止连线任务，不得重试。',
	'- `schematic_wire_execute`：只能在 `schematic_wire_plan` 成功返回、用户已确认后才能调用。必须将 `planId` 和 `connectionMethod` 原样传入，不得修改或替换。严禁先后各用一个工具分别调用 `schematic_wire_plan` 和手动 API，必须完整走 plan → execute 流程。',
	'',
].join('\n');
